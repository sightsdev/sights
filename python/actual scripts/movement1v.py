...
def leftRightRun(key, s):								#Function for going left and right on the spot
	...
	if (key == 3):									#--- turn left ---
		runMotorGroup([1,2], fwd)
		runMotorGroup([3,4], bwd)#run motors on right side forward and left side backward
		return None	
	elif (key == 4):								#--- turn right ---
		runMotorGroup([1,2], bwd)
		runMotorGroup([3,4], fwd)				#run right side back
...
def run(websocket, path):
	...
	while True:
		...
		print(buf_array)
		print("Speed: {}".format(getSpeed(buf)))
		if len(buf) > 0:
